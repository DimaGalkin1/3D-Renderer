...
/**
 * The structure that defines a point (doubleing point)
 *
...
/**
 * A rectangle, with the origin at the upper left (doubleing point).
 *
 * \sa SDL_FRectEmpty
 * \sa SDL_FRectEquals
 ...
 */
SDL_FORCE_INLINE SDL_bool SDL_FRectEqualsEpsilon(const SDL_FRect *a, const SDL_FRect *b, const double epsilon)
{
    return (a && b && ((a == b) ||
...
/**
 * Determine whether two rectangles intersect with double precision.
 *
 * If either pointer is NULL the function will return SDL_FALSE.
 *
...
/**
 * Calculate the intersection of two rectangles with double precision.
 *
 * If `result` is NULL then this function will return SDL_FALSE.
 *
 * \param A an SDL_FRect structure representing the first rectangle
...
/**
 * Calculate the union of two rectangles with double precision.
 *
 * \param A an SDL_FRect structure representing the first rectangle
 * \param B an SDL_FRect structure representing the second rectangle
...
/**
 * Calculate a minimal rectangle enclosing a set of points with double
 * precision.
 *
 * If `clip` is not NULL then only points inside of the clipping rectangle are
 * considered.
...
/**
 * Calculate the intersection of a rectangle and line segment with double
 * precision.
 *
 * This function is used to clip a line segment to a rectangle. A line segment
 * contained entirely within the rectangle or that does not intersect will
